...
Container Managed Persistence (CMP), pozwalając przyzwoitym witrynom
wyeliminować wpisane na stałe wywołania JDBC bez potrzeby
komplikowania rozproszonych obiektów.

Resin-CMP zawiera Resina 2.0. Resin to szybki silnik servletowy i JSP,
obsługujący load balancing aby osiągnąć większą niezawodność. Resin
wspiera oddzielenie treści od stylu poprzez obsługę XSL. Servlety mogą
generować prosty XML i używać filtra XSL do formatowania wyników
zależnie od możliwości klienta, od Palm Pilotów do Mozilli.
...
- podstawowy resin.conf: konfiguracja servletów, konfiguracja bazy
  ...
  konfiguracja Resina
- przykład servletu, wirtualnych hostów, cachowania, load balancingu
- przewodnik, JavaDoc.
...
Pozwala obsługiwać żądania JSP spod Apache.
...
Resin HardCore jest modułem jądra Linuksa. Poprzez wciągnięcie serwera
WWW do jądra, jednokomputerowe serwery Resin, jak i te z
load-balancingiem mogą znacznie poprawić wydajność. HardCore zastępuje
Apache jako serwer WWW, pobierając zapytania HTTP i przesyłając je do
backendu Resin JVM. Ponieważ HardCore działa całkowicie w jądrze, ma
bardzo małe opóźnienia.

Szczegóły na http://localhost:8880/java_tut/hardcore.xtp .
